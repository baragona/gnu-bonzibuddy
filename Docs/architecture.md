...
`FacialIntent.individualBrowLower` supplies independent brow-lowering channels. The fan renderer applies the imported eyebrow-down morph through a smooth left/right mask; the same deformation runs in color, shadow, and geometry-audit passes. An explicit manual override of that morph takes priority. Wink lowers only the closing eye’s brow, with the same eyelid envelope. `--validate-wink-brow` checks the actual GPU-deformed mesh for lowering, opposite-side isolation, and manual override behavior.
