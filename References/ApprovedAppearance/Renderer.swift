...
final class Renderer: NSObject, MTKViewDelegate {
    ...
    init(device: MTLDevice) throws {
        ...
        let mesh = try PolygonMesh.load(from:meshURL)
    ...
    func encode(_ command: MTLCommandBuffer, pass: MTLRenderPassDescriptor, width: Int, height: Int, at time: Double, buffer: MTLBuffer) {
        let objects = character.instances(at: time)
        ...
        var uniforms = RenderUniforms(projection:projection(width:width,height:height),light:lightMatrix,ground:rotate(character.pitch,[1,0,0])*translation([0,-0.92,0]),options:[shadowsEnabled ? 1 : 0,0.28,character.mouthOpening,0],jawAxis:normalize(objects[character.headBoneStart].model.columns.1))
