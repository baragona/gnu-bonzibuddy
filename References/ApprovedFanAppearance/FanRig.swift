...
final class FanRig {
    ...
    func instances(yaw:Float,pitch:Float,at time:Double)->[Instance] {
        ...
        func armAction(_ side:Float)->ArmPose? {
            ...
            if action == .dance { target=ArmPose(elbow:[side*0.61,0.05+0.13*sin(a*6+side),0],wrist:[side*0.93,0.2+0.23*sin(a*6+side),0.1],angle:side*1.7,spread:0.7) }
        ...
        for (i,node) in source.nodes.enumerated() {
            ...
            if !sourcePose && i==20 && action == .dance {
                let p=xyz(inherited)
                result=translation(p+[0,0.025*sin(a*6)*actionEnvelope,0])*rotate(0.06*sin(a*3)*actionEnvelope,[0,0,1])*translation(-p)*inherited
            }
            if !sourcePose {
                ...
                if i==56 {
                    ...
                    let tilt:Float=action == .think ? 0.12*actionEnvelope : 0
                ...
                if let pose=arms[side] {
                    ...
                    if i==22 || i==39 { result=aim(i==22 ? 23:40,restingUpper+de*1.5) }
                    if i==23 || i==40 { result=aim(i==23 ? 24:41,restingForearm+(dw-de)*2) }
                    if i==26 || i==42 {
                        ...
                        if action == .shrug { result=blendRotation(result,handFrame(i==26 ? 30:46,[side,0.08,0],[0,1,0.1]),pose.spread) }
