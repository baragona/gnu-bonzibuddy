...
fragment float4 fragmentMain(Varying in [[stage_in]],depth2d<float> shadow [[texture(0)]],constant Uniforms& u [[buffer(2)]],constant float4& face [[buffer(3)]]) {
    ...
    if (in.eyeUV.z>0.5) {
        ...
        float closure=clamp(face.x,0.0,1.0);
