...
if CommandLine.arguments.contains("--validate-presence") {
    do {try validatePresence()} catch {fputs("Presence validation failed: \(error)\n",stderr);exit(1)}
    exit(0)
}
if CommandLine.arguments.contains("--validate-vine-entrance") {
    do {try validateVineEntrance()} catch {fputs("Vine entrance validation failed: \(error)\n",stderr);exit(1)}
    exit(0)
}
if CommandLine.arguments.contains("--validate-actor-placement") {
    do {try validateActorPlacement()} catch {fputs("Actor placement validation failed: \(error)\n",stderr);exit(1)}
    exit(0)
}
...
if CommandLine.arguments.contains("--validate-planted-feet") {
    do {try validatePlantedFeet()} catch {fputs("Planted feet validation failed: \(error)\n",stderr);exit(1)}
    exit(0)
}
...
if CommandLine.arguments.contains("--validate-writing-transfers") {
