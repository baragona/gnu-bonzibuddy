...
func validateFanActions() throws {
    ...
    for action in faceOnly ? [] : Action.allCases {
        ...
        for frame in 0..<frames {
            ...
            if frame==min(15,frames/2) || frame==frames/2 || (action == .clap && frame<=6) || (action == .shrug && [5,7,9,22,27,33].contains(frame)) || (action == .think && [5,10,20,45,55,60].contains(frame)) || (action == .dance && frame%10==0) {
