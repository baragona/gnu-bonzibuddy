...
func validateFanTransitions() throws {
    ...
    for from in Action.allCases { for to in Action.allCases {
        ...
        for frame in 1...30 {
            ...
            if !from.changesFacing && !to.changesFacing && !from.changesStance && !to.changesStance { footDrift=max(footDrift,error(before,pose,Array(12...19))) }
