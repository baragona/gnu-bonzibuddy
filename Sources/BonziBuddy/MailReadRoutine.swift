...
enum MailReadRoutine {
    ...
    static func sample(at t:Double)->RoutinePose {sample(at:t,flapOpening:nil)}
    static func sample(at t:Double,flapOpening:Float?,flapGrip:Float? = nil)->RoutinePose {
        ...
        let opened=flapOpening ?? (RoutineLibrary.smooth(t,1.05,1.35)*(1-RoutineLibrary.smooth(t,1.50,1.85)))
        ...
        let grip=flapGrip ?? (RoutineLibrary.smooth(t,0.90,1.05)*(1-RoutineLibrary.smooth(t,1.85,2.10)))
        var left=SIMD3<Float>(-0.38,0,0.08)
        left += (hinge+rotation.act(SIMD3<Float>(0.42,0.12,0.15))-left)*grip
        left=SIMD3<Float>(-0.10,-0.03,0.06)+(left*paperScale-SIMD3<Float>(-0.10,-0.03,0.06))*held
        let handRotation=rotation*simd_quatf(angle:Float.pi*grip,axis:[0,0,1])
        let hands:[HandSide:HandIntent]=[
            .left:HandIntent(wrist:origin+left,fingers:handRotation.act([1,0,0]),palm:handRotation.act([0,0,-1]),weight:active,grip:0.18),
            .right:HandIntent(wrist:origin+SIMD3<Float>(0.38,0,0.08)*paperScale,fingers:[-1,0,0],palm:[0,0,-1],weight:held,grip:0.18)
        ]
